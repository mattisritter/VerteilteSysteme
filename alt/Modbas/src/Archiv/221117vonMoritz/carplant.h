...
*/
  void step(const InputsType& u, OutputsType& y, const float dt)
  {
    ...
    float beta = 0.0F;
    if(std::abs(x.at(0)) < 0.2F/*m/s*/) /*calculating beta for different models*/
    {
    ...
    y.at(1) = x.at(2) - p->lr * std::cos(x.at(3));
    y.at(2) = x.at(3);
    y.at(3) = beta;
    y.at(4) = x.at(0);
    y.at(5) = x.at(6);
...
*/
  void operator()(const StatesType& x, StatesType& xd, float t)
  {
    if(std::abs(x.at(0)) < 0.2F/*m/s*/)
    {
      ...
      xd.at(3)= x.at(0) / (p->l) * std::tan(p->deltaMax * uDelayed.at(2));
    ...
    else
    {
      ...
      if(x.at(0) > 0.0F)
      {
        alphaf = -1.0F * std::atan((x.at(5) + p->lf * x.at(4)) / x.at(0) + deltaT);
      }
      else if(x.at(0) < 0.0F)
      {
      ...
      if(x.at(0) > 0.0F)
      {
        alphar = -1.0F * std::atan((x.at(5) - p->lr * x.at(4)) / x.at(0));
      }
      else if(x.at(0) < 0.0F)
      {
...
parameterized with states vector type */
  ...
  //unnötig- std::deque<float>u1Deque;// = {0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F,0.0F}; //Prefill with zeros, 30 times
