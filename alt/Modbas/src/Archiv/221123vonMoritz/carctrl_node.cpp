 ...
 */
     ...
     samplingTime(samplingTimeArg) // initializes the member samplingTime
   {
     ...
     outputsExtSub = node.subscribe("/mad/caroutputsext", 1, &CarCtrlNode::inputsCallback, this);
     driveManeuverSub = node.subscribe("/mad/car0/navi/maneuver", 1, &CarCtrlNode::inputsCallback, this);
 ...
 */
   void step()
   {
     ...
     model.step(ek, uk, samplingTime);
     // copy the output signal to the message
     if (uk >= 1)       /*limit control signal to 1*/
     {
       carInputsMsg.pedals = 1;
     }
     else if (uk <= -1) /*limit controll signal to -1*/
     {
       carInputsMsg.pedals = -1;
     }
     else                     /*Pass controll signal*/
     {
       carInputsMsg.pedals = uk;
     }
     ...
     carInputsMsg.steering = 0.7; //Value according to book
 ...
 private:
   ...
   ModelType::InputsType ek { { 0.0F, 0.0F } }; /**< The input signal of the model */
   ModelType model {};
 ...
 */
   void inputsCallback(const madmsgs::CarInputs& msgOutputsExt, const madmsgs::DriveManeuver& msgDriveManeuver)
   {
     // copy the input signal to the member variable u
     ek.at(0) = msgOutputsExt.v;        //Actual speed y
     ek.at(1) = msgDriveManeuver.vmax;  //Wished speed w
   }
