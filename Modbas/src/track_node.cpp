...
//    getchar();
  ...
  std::shared_ptr<modbas::TrackPose> roundaboutPose;
  track.addPose(pose);
  // straight segment 0
  ...
  if (!track.finalizeCircuit()) {
    ROS_ERROR("finalizing outer lap failed");
  }
  // roundabout
  // entrance 14 and 15
  track.addSegment(std::make_shared<modbas::CircleSegment>(
  ...
  track.addSegment(std::make_shared<modbas::CircleSegment>(
                   pose, width, radius, 0.5F * modbas::Utils::pi ));
  roundaboutPose = pose;
  // three quater circle 17
  track.addSegment(std::make_shared<modbas::CircleSegment>(
                  pose, width, radius, 1.5F * modbas::Utils::pi ));
  if (!track.finalizeCircuit()) {
    ROS_ERROR("finalizing roundabout failed");
  }
  // exit 18 and 19
  track.addSegment(std::make_shared<modbas::StraightSegment>(
                   roundaboutPose, width, 0.5F * a2 - radius - 0.5F * width));
  track.addSegment(std::make_shared<modbas::CircleSegment>(
                   roundaboutPose, width, radius, 0.5F * modbas::Utils::pi ));

//  if (!track.finalizeCircuit()) {
//    ROS_ERROR("finalizing inner lap failed");
//  }
