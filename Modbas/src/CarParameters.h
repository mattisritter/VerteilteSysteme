...
public:
    ...
    // initial conditions v0, s10, s20, psi0, added: w0, vc20, x0
    stateext_t x0 { { 0.0F, 1.0F, 0.2F, 0.0F, 0.0F, 0.0F, 0.0F } };
    ...
    const float Ti = 246.83e-3F; // Time constant for integration [s]
    const float kr = 0.344F;  // gain for speed controll[s/m]

    //Position Controller
    const float kp = 1.0F;

    //Path Controller
    const float pathTw = 300e-3F;
